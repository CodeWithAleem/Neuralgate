"""NeuralGate — Model Execution Layer (with conversation history)

All providers now accept a messages array for multi-turn conversations.
"""
...
def _build_messages(query, history=None):
    ...
    msgs = []
    if history:
        for h in history[-10:]:  # Last 10 messages max to avoid token limits
...
async def call_cohere(query, model_id, api_key, history=None):
    """Cohere API v2 — with conversation history and fixed response parsing."""
    model_name = MODELS[model_id]["api_model"]
    try:
...
async def call_mock(query, model_id, api_key, history=None):
    ...
    if history:
        text = f"[Mock] Continuing conversation (turn {len(history)+1}). You asked: {query[:80]}"
